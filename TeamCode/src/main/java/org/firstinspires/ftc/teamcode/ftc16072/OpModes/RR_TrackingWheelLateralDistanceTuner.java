...
@Config
@TeleOp()
public class RR_TrackingWheelLateralDistanceTuner extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (!isStopRequested() && !tuningFinished) {
            ...
            robot.nav.setDrivePower(vel);
            robot.nav.updatePoseEstimate();

            double heading = robot.nav.getPoseEstimate().getHeading();
        ...
        lateralDistance = robot.leftPod.getPose().getY() - robot.rightPod.getPose().getY();
        telemetry.addLine("Effective LATERAL_DISTANCE: " +
                (headingAccumulator / (NUM_TURNS * Math.PI * 2)) * lateralDistance);
